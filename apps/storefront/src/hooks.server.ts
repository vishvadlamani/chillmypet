...
import { createCommerce, createDirectory } from '@chillmypet/commerce';
...
/** Guards against DB content reaching the page as markup. */
...
export const handle: Handle = async ({ event, resolve }) => {
	...
	// a configured default so development doesn't need a hosts-file entry.
	...
	return resolve(event, {
		transformPageChunk: ({ html }) =>
			html
				...
				.replace('%meta_pixel%', store.metaPixelId ? pixelSnippet(store.metaPixelId) : '')
				.replace(
					'%meta_domain_verification%',
					store.metaDomainVerification ? verificationTag(store.metaDomainVerification) : ''
				)
	});
};
