<script lang="ts">
	// Stand-in artwork keyed to the variant colour. Replace with real product
	// photography per colour before launch — see README.
	let {
		hex,
		label,
		class: className = ''
	}: { hex: string; label: string; class?: string } = $props();
